...
class DynamicsNetworkNoPrefix(nn.Module):
    def __init__(
        self,
        num_blocks,
        num_channels,
        ...
        self.bn_reward = nn.BatchNorm2d(reduced_channels_reward, momentum=momentum)
        self.block_output_size_reward = block_output_size_reward
        self.bn_value_prefix = nn.BatchNorm1d(self.block_output_size_reward, momentum=momentum)
        self.fc = mlp(self.block_output_size_reward, fc_reward_layers, full_support_size, init_zero=init_zero, momentum=momentum)
...
class EfficientZeroNet(BaseNet):
    def __init__(
        ...
        if self.use_value_prefix:
            ...
            self.dynamics_network = DynamicsNetworkNoPrefix(
                num_blocks,
                num_channels + 1,
                reduced_channels_reward,
                fc_reward_layers,
    ...
    def dynamics(self, encoded_state, reward_hidden, action):
        ...
        if self.use_value_prefix:
            ...
            next_encoded_state, value_prefix = self.dynamics_network(x)
